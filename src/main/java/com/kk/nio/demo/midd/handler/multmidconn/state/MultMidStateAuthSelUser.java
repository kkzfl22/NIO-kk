 ...
 * @author liujun
 */
public class MultMidStateAuthSelUser implements MultMidStateInf {
			...
			try {
				...
				if (readPosition > 0) {
					...
					if (length == byteBuffer.position()) {
						...
						System.out.println("中间件,响应登录成功之后的心跳消息查询请求:readPosition:" + readPosition + ",length:" + length);
						// 设置写入的大小
		...
		if (iostateContext.getWriteLock().compareAndSet(false, true)) {
			try {
				...
				if (readPos > 0) {
					...
					if (rsp == readPos) {
						System.out.println("中间件,响应登录成功之后的心跳消息查询结果:" + rsp);
