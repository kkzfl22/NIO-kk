package com.kk.nio.demo.midd.memory;

import java.nio.ByteBuffer;

/**
 * java 内存池的实现 源文件名：MemoryPool.java 文件版本：1.0.0 创建作者：liujun 创建日期：2016年12月19日
 * 修改作者：liujun 修改日期：2016年12月19日 文件描述：TODO 版权所有：Copyright 2016 zjhz, Inc. All
 * Rights Reserved.
 */
public class MemoryPool {
	 ...
	 * 内存池实例对象
	 */
	private static final MemoryPool MEMPOOLINSTALL = new MemoryPool(1024 * 1024, 1024 * 1024 * 16, (short) 4);
	 ...
	 *            内存池的个数
	 */
	public MemoryPool(int chunkSize, int memorySize, short ItemPoolSize) {
		CHUNK_SIZE = chunkSize;
		// 进行每个内存页的初始化
		POOL = new PageMemory[ItemPoolSize];
		// 进行每个chunk的页面的分配内存操作
