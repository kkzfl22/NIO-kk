 ...
 * @author liujun
 */
public class MysqlMultReactor extends Thread {
	...
	public MysqlMultReactor(ExecutorService executor) throws IOException {
	...
	public void rigisterNewConn(SocketChannel socket) throws IOException {
		...
		new MysqmidIOHandler(select, socket);
	}
	...
	public void run() {
		...
		while (true) {
			...
			for (SelectionKey selectionKey : selectKey) {
				// 从attach对象中获取处理对象信息
				MysqlIOHandlerBase handler = (MysqlIOHandlerBase) selectionKey.attachment();
				if (null != handler) {
					// 提交线程池处理
					handler.run();
