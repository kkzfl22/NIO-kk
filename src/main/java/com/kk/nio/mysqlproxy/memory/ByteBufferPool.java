package com.kk.nio.mysqlproxy.memory;

import java.nio.ByteBuffer;

/**
 * java 内存池的实现
 * @since 2017年6月23日 下午4:19:11
 * @version 0.0.1
 * @author liujun
 */
public class ByteBufferPool {
	 ...
	 * @字段说明 pool
	 */
	private final ByteBufferPageMemory[] POOL;
	 ...
	 * 内存池实例对象
	 */
	private static final ByteBufferPool MEMPOOLINSTALL = new ByteBufferPool(1024 * 1024, 1024 * 1024 * 16, (short) 4);

	public static ByteBufferPool Instance() {
		return MEMPOOLINSTALL;
	}
	 ...
	 *            内存池的个数
	 */
	public ByteBufferPool(int chunkSize, int memorySize, short ItemPoolSize) {
		CHUNK_SIZE = chunkSize;
		// 进行每个内存页的初始化
		POOL = new ByteBufferPageMemory[ItemPoolSize];
		// 进行每个chunk的页面的分配内存操作
		for (int i = 0; i < ItemPoolSize; i++) {
			POOL[i] = new ByteBufferPageMemory(ByteBuffer.allocateDirect(memorySize), CHUNK_SIZE);
		}
	}
	 ...
	 * 
	 * @param size
	 *            需要的内存大小,最好CHUNK_SIZE的倍数，以方便 内存的回收利用
	 * @return
	 * @创建日期 2016年12月19日
	 */
	public ByteBuffer allocate(int size) {
		...
		// 取得内存页信息
		ByteBufferPageMemory page = null;
		for (ByteBufferPageMemory pageMemory : POOL) {
